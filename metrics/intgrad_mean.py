...
def getgrad(model:torch.nn.Module, grad_dict:dict, step_iter=0, step = 1):
...
def cal_interval(model:torch.nn.Module, grad_dict:dict):
...
def caculate_intgrad_mean(grad_dict, bins = None):
    ...
    for j, modname in enumerate(grad_dict.keys()):
        ...
        for b in bins:
            ...
            tmpsum = np.mean(grad_mean[stable_idx] / grad_std[stable_idx])
            if tmpsum > maxtmpsum:
                maxtmpsum = tmpsum
        tmpsum = maxtmpsum
...
def compute_intgrad_mean(network, trainloader, lossfunc, bincount=None):
    grad_dict= {}
    network.train()
    batch_size = 0
    network.cuda()
    ...
    for i, batch in enumerate(trainloader):
        ...
        step = loss - 2.25
        meanloss += loss
        grad_dict = getgrad(network, grad_dict, i, (step))
        batch_size += 1        

    bins, idx, subbins, subidx = cal_interval(network, grad_dict)
    res1 = caculate_intgrad_mean(grad_dict, [subbins])

    ...
    def __init__(self, in_channels=None, out_channels=None, stride=None, bottleneck_channels=None ,sub_layers=None, kernel_size=None,
        ...
        for i in range(self.sub_layers):
            ...
            if not self.no_reslink:
                if last_channels != out_channels:
                    res_str = 'ResBlockProj({},{},{})RELU({})'.format(last_channels,current_stride,inner_str, out_channels)
                else:
...
class SuperResAtten(PlainNetSuperBlockClass):
    def __init__(self, in_channels=None, out_channels=None, stride=None, bottleneck_channels=64 ,sub_layers=1, kernel_size=None,
        ...
        self.no_BN = no_BN
        self.use_se = use_se
        self.dim_head = bottleneck_channels
        if self.use_se:
            print('---debug use_se in ' + str(self))
        ...
        attn_dim_out = self.dim_head * heads
        ...
        for i in range(self.sub_layers):
            ...
            # projection 
            inner_str += 'ConvKX({},{},{},{})'.format(last_channel, attn_dim_in, 1, 1)
            inner_str += 'ConvDW({},{},{})'.format(attn_dim_in, 3, current_stride)
            inner_str += 'BN({})'.format(attn_dim_in)

            #attention net
            inner_str += 'RELU({})'.format(attn_dim_in)
            inner_str += 'MSA({},{},{})'.format(attn_dim_in, heads, self.dim_head)
            ...
            #res link
            if last_channel != self.out_channels:
                res_str = 'ResBlockProj({},{},{})RELU({})'.format(last_channel, current_stride, inner_str, out_channels)
            else:
                res_str = 'ResBlock({},{})RELU({})'.format(last_channel,inner_str, self.out_channels)
            full_str += res_str
            last_channel = self.out_channels
            current_stride = 1
        pass
    ...
    def __repr__(self):
        return type(self).__name__ + '({}|in={},out={},stride={},bottleneck={},sublayers={})'.format(
            self.block_name, self.in_channels, self.out_channels, self.stride, self.bottleneck_channels, self.sub_layers
        )

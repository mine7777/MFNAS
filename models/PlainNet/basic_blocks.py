...
class AvgPool(PlainNetBasicBlockClass):
    def __init__(self, kernel_size, stride, no_create=False, **kwargs):
        ...
        self.stride = stride
        if not no_create:
            self.netblock = nn.AvgPool2d(kernel_size=kernel_size, stride=stride, padding=self.padding)

    def forward(self, x):
        return self.netblock(x)

    def __str__(self):
        return type(self).__name__ + '({},{})'.format(self.kernel_size, self.stride)
    ...
    @classmethod
    def create_from_str(cls, s, no_create=False, **kwargs):
        assert AvgPool.is_instance_from_str(s)
        idx = _get_right_parentheses_index_(s)
        assert idx is not None
        param_str = s[len('AvgPool('):idx]
        ...
        param_str_split = param_str.split(',')
        kernelsize = int(param_str_split[0])
        stride = int(param_str_split[1])
        return AvgPool(kernel_size=kernelsize, stride=stride,
                               block_name=tmp_block_name, no_create=no_create), s[idx + 1:]
    ...
    def __init__(self, out_channels=None, kernel_size=None, stride=None, copy_from=None,
        ...
        if copy_from is not None:
            ...
            if no_create or self.in_channels == 0 or self.out_channels == 0 or self.kernel_size == 0 \
                ...
                self.netblock = nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels,
                                          kernel_size=self.kernel_size, stride=self.stride,
                                          padding=self.padding, bias=True, groups=self.in_channels)
    ...
    def __init__(self, block_list, in_channels=None, stride=None, no_create=False, **kwargs):
        ...
        if self.stride == 2:
            self.proj = nn.Sequential(
                ...
                nn.Conv2d(self.in_channels, self.out_channels, 1, 1, bias=False),
                nn.BatchNorm2d(self.out_channels),
                )
        else:
            self.proj = nn.Sequential(
...
class MSA(PlainNetBasicBlockClass):
    def __init__(self, dim, heads=4, dim_head=64, **kwargs):
        ...
        self.netblock = Attention(self.dim, self.heads, self.dim_head)

    def forward(self, x):
        return self.netblock(x)

    def __str__(self):
        return 'MSA({},{},{})'.format(self.dim, self.heads, self.dim_head)

    def __repr__(self):
        return 'MSA({}|{},{},{})'.format(self.block_name, self.dim, self.heads, self.dim_head)
    ...
    def get_FLOPs(self, input_resolution):
        #self.in_channels * self.out_channels * self.kernel_size ** 2 * input_resolution ** 2 // self.stride ** 2 // self.groups
        N2 = input_resolution ** 2
        flops_to_qkv = self.dim * self.inner_dim * 3 * 3 ** 2 * N2
        flops_scale = N2 * self.heads * self.dim_head
        flops_matmul = self.heads * (N2 * N2 * self.dim_head) * 2 + N2 * N2 * self.heads
        return flops_matmul + flops_to_qkv + flops_scale

    #FIXME: modify
    def get_model_size(self):
        # self.in_channels * self.out_channels * self.kernel_size ** 2 // self.groups
        return self.dim * self.inner_dim * 3
    ...
    @classmethod
    def create_from_str(cls, s, no_create=False, **kwargs):
        assert MSA.is_instance_from_str(s)
        idx = _get_right_parentheses_index_(s)
        assert idx is not None
        param_str = s[len('MSA('):idx]
        # find block_name
        tmp_idx = param_str.find('|')
        if tmp_idx < 0:
            tmp_block_name = 'uuid{}'.format(uuid.uuid4().hex)
        ...
        param_str_split = param_str.split(',')
        dim = int(param_str_split[0])
        heads = int(param_str_split[1])
        dim_head = int(param_str_split[2])
        return MSA(dim=dim, heads=heads, dim_head=dim_head, block_name=tmp_block_name), s[idx + 1:]

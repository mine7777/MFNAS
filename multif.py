...
from searchalg import mutate


def multif(api, N=25, n=5, P=[4], R=[1], max_flops=0.1, max_params=0.4, max_time=2, n_random=0, analyzer=None, metric=None, acc_log=None):
    ...
    while step < 100:
        ...
        while True:
            ...
            for r, p in zip(R, P):
                ...
                # top_mutated += get_random_population(api, 1)
        ...
        if acc_log is not None:
            acc_log.append(multif_gettop(api, exemplars, history, start, metrics=metrics, max_flops=max_flops, max_params=max_params, itper=step/100))
    ...
    print(f'multif step: {step}')

    top1_hist = multif_info(api, exemplars, history, start, metrics=metrics, max_flops=max_flops, max_params=max_params)

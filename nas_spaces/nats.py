...
class NATS_SSS(NASSpaceBase):

    _all_width = {'8', '16', '24', '32', '40', '48', '56', '64'}
    ...
    def __init__(self, path: str, dataset: str, verbose: bool = False, metric_root=None):
        ...
        # Init wrapped API
        self._official_api = create(path, 'sss', fast_mode=True, verbose=verbose)
    ...
    def __getitem__(self, item: int) -> Exemplar:
        ...
        config = self._official_api.get_net_config(item, self._dataset)
        return Exemplar(self, item, genotype=config['channels'])
    ...
    def get_accuracy(self, spec: Exemplar, epochs: int = 200) -> float:
        index = spec.idx
        info = self._official_api.get_more_info(index, self._dataset, hp="90", is_random=False)
        return info['test-accuracy']
    ...
    @staticmethod
    def _get_different_gene(gene):
        suitable = copy(NATS_SSS._all_width)
        suitable.remove(gene)
        gene = np.random.choice(list(suitable))
        return gene
    ...
    def mutation(self, exemplar: Exemplar, R: int = 1) -> Exemplar:
        ...
        for _ in range(R):
            # Level choice weighted by the number of genes per level
            mutating_geneidx = np.random.randint(5)
            mutating_gene = genotype[mutating_geneidx]
            genotype[mutating_geneidx] = self._get_different_gene(mutating_gene)

        mutated_genotype = ':'.join(genotype)

        index = self.get_index(mutated_genotype)
        return self[index]
    ...
    def crossover(self, exemplars: Sequence[Exemplar]):
        ...
        parent1 = genotype1.split(':')
        parent2 = genotype2.split(':')

        new_genotype = []
        for i in range(5):
            choice = np.random.binomial(1, 0.5)
            if choice == 0:
                new_genotype.append(parent1[i])
            else:
                new_genotype.append(parent2[i])
        new_genotype = ':'.join(new_genotype)

        index = self.get_index(new_genotype)
        return self[index]
